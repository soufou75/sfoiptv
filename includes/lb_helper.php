...
function get_api_data($data_info){
    ...
    $APP_NAME = 'ONLINE_LIVE_TV_APP';
    
    $data_json = $data_info;
    $data_arr = json_decode(urldecode(base64_decode($data_json)), true);
    if($data_arr['application_id']==PACKAGE_NAME){
        if (!file_exists('api.php')){
            //$data['data'] = array('success' => '0', "MSG" => "Invalid.");
            $set[$APP_NAME][] = array('success' => '0', "MSG" => LB_TEXT_API_FILE_MISSING);   
        ...
        //$data['data'] = array('success' => '0', "MSG" => "Invalid.");
        $set[$APP_NAME][] = array('success' => '0', "MSG" => LB_TEXT_INVALID_PACKAGE_NAME);   
        header( 'Content-Type: application/json; charset=utf-8' );
        echo $val= str_replace('\\/', '/', json_encode($set,JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT));
        exit();
    }
...
function call_api($data){
    ...
    curl_setopt($curl, CURLOPT_URL, "https://api.nemosofts.com/v2/api_helper.php");
